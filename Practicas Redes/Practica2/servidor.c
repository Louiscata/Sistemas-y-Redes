...
int main(int argc, char *argv[]) {
    ...
    while (1)
    {
        ...
        /* char ip_conexion_texto[INET_ADDRSTRLEN];
        inet_ntop(AF_INET, &direccion_socket_servidor.sin_addr.s_addr, ip_conexion_texto, INET_ADDRSTRLEN);
        printf("IP del servidor: %s\nPuerto del servidor: %d\n",
            ip_conexion_texto, 
            ntohs(direccion_socket_servidor.sin_port)); */
