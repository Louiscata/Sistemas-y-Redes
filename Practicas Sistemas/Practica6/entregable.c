...
int main(int argc, char *argv[])
{
    ...
    {
        // printf("Aquí el padre. Caracter: [%c]\n", ((char *)memoria_entrada)[i]);
    ...
    printf("Primera mitad transformada:\n");
    for (int k = 0; k < (int)salidaStats.st_size; k++)
    {
        isdigit(buffer[k]) ? printf("%c", buffer[k]) : printf("%c", buffer[k]);
    }
    printf("\n");
